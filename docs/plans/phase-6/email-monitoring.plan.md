...
Gmail Inbox
    |
    v
Gmail MCP Server (gmail_search_messages)
    |
    v
Classification Agent (Gemini)
    |
    +-- Urgent (interview request, time-sensitive) --> Telegram alert
    +-- Important (recruiter reply, offer)          --> Telegram alert
    +-- Informational (rejection, confirmation)     --> Daily digest
    +-- Irrelevant                                  --> Skip
```
